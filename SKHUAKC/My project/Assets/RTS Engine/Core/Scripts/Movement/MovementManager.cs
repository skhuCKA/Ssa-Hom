...
namespace RTSEngine.Movement
{
    public class MovementManager : MonoBehaviour, IMovementManager
    {
        ...
        public ErrorMessage SetPathDestination(IReadOnlyList<IEntity> entities, Vector3 destination, float offsetRadius, IEntity target, bool playerCommand)
        {
        ...
        public ErrorMessage SetPathDestinationLocal(IReadOnlyList<IEntity> entities, Vector3 destination, float offsetRadius, IEntity target, bool playerCommand)
        {
            if (!entities.IsValid())
            {
            ...
            // Only one entity to move? use the dedicated method instead!
                ...
                return SetPathDestinationLocal(entities[0], destination, offsetRadius, target, new MovementSource { playerCommand = playerCommand });
            ...
            var sortedMvtSources = RTSHelper.SortEntitiesByCode(
                entities,
                entity => entity.CanMove(playerCommand))
                .Values
